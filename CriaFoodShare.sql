...
CREATE TABLE fs_t_cliente (
    ...
    tp_cliente   VARCHAR2(10) NOT NULL
);
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'RECEPTOR' ) THEN
        raise_application_error(-20223, 'FK FS_T_RECEPTOR_CLIENTE_FK in Table FS_T_RECEPTOR violates Arc constraint on Table FS_T_CLIENTE - discriminator column TP_CLIENTE doesn''t have value ''RECEPTOR''');
    END IF;
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'DOADOR' ) THEN
        raise_application_error(-20223, 'FK FS_T_DOADOR_CLIENTE_FK in Table FS_T_DOADOR violates Arc constraint on Table FS_T_CLIENTE - discriminator column TP_CLIENTE doesn''t have value ''DOADOR''');
    END IF;
